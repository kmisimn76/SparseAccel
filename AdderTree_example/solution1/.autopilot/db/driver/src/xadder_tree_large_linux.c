...
typedef struct {
    ...
    XAdder_tree_large_uio_map maps[ MAX_UIO_MAPS ];
} XAdder_tree_large_uio_info;

/***************** Variable Definitions **************************************/
static XAdder_tree_large_uio_info uio_info;
...
static int uio_info_read_name(XAdder_tree_large_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XAdder_tree_large_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XAdder_tree_large_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XAdder_tree_large_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XAdder_tree_large_Initialize(XAdder_tree_large *InstancePtr, const char* InstanceName) {
	XAdder_tree_large_uio_info *InfoPtr = &uio_info;
