...
#endif
#ifndef	  DATA_L2_SIZE
 #define   DATA_L2_SIZE 2304 //(2304/PORT_NUM) //2048//180 //256 //2048 / PORT_NUM + a
#endif
#ifndef OUTPUT_L2_SIZE
 #define OUTPUT_L2_SIZE 1600 //(1600/PORT_NUM) //1568//98 //196 //1568 / PORT_NUM
#endif
#define WEIGHT_L1_SIZE 128
#define   DATA_L1_SIZE 288 //(288/PORT_NUM) //256//32 //256/PORT //196 //49
#define OUTPUT_L1_SIZE 288 //(288/PORT_NUM) //256//32 //256/PORT //196 //49
...
extern "C" {
#endif
void runWeightL2toL1(DPTYPE weight_l1[WEIGHT_L1_SIZE][PORT_C][ARRAY_K][BLOCK_SIZE], DPTYPE weight_l2[WEIGHT_L2_SIZE][PORT_C][ARRAY_K],
		...
		for (int pt = 0; pt < PORT_C; pt++) {
			...
			for (int wib = 0; wib < ARRAY_W/BLOCK_SIZE; wib++) { // place unroll to inner-most
				...
				for (int bl = 0; bl < BLOCK_SIZE; bl++) { // place unroll to inner-most
					...
					int c = co*(C_L1/PORT_C) + cip;
					int h = (ho) + r;
					int w = wo + (((wi+s)>=ARRAY_W)?(1):(0)); // error when s >= ARRAY_W ?
...
void runSIMD_bitvec(
		DPTYPE weight_l1[WEIGHT_L1_SIZE][PORT_C][ARRAY_K][BLOCK_SIZE],
		...
		LOOP_CORE_W: for (int wib = 0; wib < ARRAY_W/BLOCK_SIZE; wib++) {
			...
			for (int ki = 0; ki < ARRAY_K; ki++) {
				...
				LOOP_CORE_BLOCK: for (int bl = 0; bl < BLOCK_SIZE; bl++) {
					...
					weight_reg[bl] = weight_l1[addr][pt][ki][bl];
					MACTYPE tmp;
					#pragma HLS BIND_OP variable=tmp op=mul impl=dsp
					tmp = data_reg[bl] * weight_reg[bl];
					if(valid_input) {
		...
		for(unsigned int h = 0; h < H_in_L2; h++) {
			...
			for(unsigned int wo = 0; wo < CEIL(W_in_L2,ARRAY_W); wo++) {
				...
				for(unsigned int wi = 0; wi < ARRAY_W; wi++) { // TODO: split VECSIZE -> (VECSIZE/ARRAY_C) / ARRAY_C
					...
					uint global_chw = ((cmo*C_L2+c)*WH_in*CEIL(WH_in,ARRAY_W) + (hmo*H_L2+h)*CEIL(WH_in,ARRAY_W) + (wmo*(W_L2/ARRAY_W)+wo))*ARRAY_W + wi;
		...
		for (unsigned int h = 0; h < H_L2; h++) { // FIXME: Occur error when H_L2 cannot dividied by STREAM_BUFFER_WIDTH
			...
			OUTPUT_DRAM_STREA_IN: for (unsigned int wo = 0; wo < W_L2/ARRAY_W; wo++) {
				...
				for (unsigned int wi = 0; wi < ARRAY_W; wi++) { // TODO: split VECSIZE -> (VECSIZE/ARRAY_K) / ARRAY_K
					#pragma HLS unroll
					unsigned int global_khw = ((kmo*K_L2+k)*WH*(WH/ARRAY_W) + (hmo*H_L2+h)*(WH/ARRAY_W) + (wmo*(W_L2/ARRAY_W)+wo))*ARRAY_W + wi;
...
void Conv_sysarr(
	...
	for (unsigned int ko = 0; ko < K/ARRAY_K; ko++) {
		for (unsigned int ki = 0; ki < ARRAY_K; ki++) { // TODO: split VECSIZE -> (VECSIZE/ARRAY_K) / ARRAY_K
			#pragma HLS loop_tripcount min=1 max=1
			for (int wh = 0; wh < WH*(WH/ARRAY_W); wh++) {
				#pragma HLS loop_tripcount min=49 max=49
				for (unsigned int wi = 0; wi < ARRAY_W; wi++) { // TODO: split VECSIZE -> (VECSIZE/ARRAY_K) / ARRAY_K
					unsigned int global_khw = ((ko*ARRAY_K+ki)*WH*(WH/ARRAY_W)+wh)*ARRAY_W + wi;
...
#pragma HLS stable variable=weight_l2
		...
		#pragma HLS loop_tripcount min=3 max=3
			...
			DPTYPE weight_l1[WEIGHT_L1_SIZE][PORT_C][ARRAY_K][BLOCK_SIZE]; //BLOCK_SIZE: duplication
			DPTYPE data_l1[DATA_L1_SIZE][PORT_C][ARRAY_W];
			#ifdef INPUT_SPARSE
			short data_l1_bitvec[DATA_L1_SIZE][PORT_C][ARRAY_W/BLOCK_SIZE];
			short data_l1_length[PORT_C][ARRAY_W/BLOCK_SIZE];
