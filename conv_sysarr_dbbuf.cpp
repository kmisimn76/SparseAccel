...
void runWeight2Reg(DPTYPE weight_regfile[ARRAY_K][ARRAY_C], uint C,
		uint RS, uint ko, uint co, uint r, uint s) {
...
#pragma HLS pipeline //must be pipelined for dataflow, (and ARRAY_K & ARRAY_C may be small)
			int k = (ko * ARRAY_K + ki);
			int c = (co * ARRAY_C + ci);
			weight_regfile[ki][ci] = weight_l2[k * C * RS
...
void runL2toL1(DPTYPE data_l1buf[512][ARRAY_C], uint H_TILE,
...
void runSysArr(DPTYPE weight_regfile[ARRAY_K][ARRAY_C], DPTYPE data_l1buf[512][ARRAY_C], uint H_TILE, uint W_TILE,
		uint ko) {
//#pragma HLS DATA_PACK variable=weight_regfile_fifo
//#pragma HLS DATA_PACK variable=data_l1buf_fifo
...
#pragma HLS loop_tripcount min=4 max=4
		...
		bias_l1[ko][ki] = (DPTYPE) bias_tmp.data(7, 0);
		}
	}
...
#pragma HLS ARRAY_PARTITION variable=data_l1buf dim=2 complete // Register
							...
							runWeight2Reg(weight_regfile, C, RS, ko, co, r, s);
							//==================================================== Input L2 -> L1
							//runL2toL1(data_l1buf_fifo, H_TILE, W_TILE, co, ho, wo, r, s, WH_in);
							runL2toL1(data_l1buf, H_TILE, W_TILE, co, ho, wo, r, s, WH_in);
							//==================================================== PE Array
							//runSysArr(weight_regfile_fifo, data_l1buf_fifo, H_TILE, W_TILE, ko);
							runSysArr(weight_regfile, data_l1buf, H_TILE, W_TILE, ko);
