...
#include "Tasks/ConvDenseOutputStationaryTask.h"
...
void ConvTask::setLayerParamAndBufSize(void* conv_layer_info)
{
	...
	if(layer_info.is_test_layer){
		...
		param.TILESIZE_K = 8; //// is allowed(not matched with W_L1)
...
void ConvTask::reorderInputs() {
	...
	for (unsigned int ko = 0; ko < param.K/VEC_SIZE; ko++) {
		for (unsigned int ki = 0; ki < VEC_SIZE; ki++) {
			...
			unsigned int device_k = (ko)*VEC_SIZE + ki;
	    ...
	    for (int rmo = 0; rmo < param.L2_TILENUM_R; rmo++) {
	     for (int smo = 0; smo < param.L2_TILENUM_S; smo++) {
	      for (unsigned int co = 0; co < (param.C_L2 / ARRAY_C); co++) {
	       for (unsigned int ko = 0; ko < param.K_L2; ko++) {
		for (unsigned int ro = 0; ro < param.R_L2; ro++) {
		 for (unsigned int so = 0; so < param.S_L2; so++) {
		  for (unsigned int ci = 0; ci < ARRAY_C; ci++) {
			unsigned int origin_kcsrs = ((kmo*param.K_L2+ko)*param.C*param.R*param.S + (cmo*param.C_L2+(co*ARRAY_C+ci))*param.R*param.S + (rmo*param.R_L2+ro)*param.S + (smo*param.S_L2 + so));
			unsigned int device_kcrs = ((cmo*(param.L2_TILENUM_K)*(param.L2_TILENUM_R)*(param.L2_TILENUM_S)+rmo*(param.L2_TILENUM_S)*(param.L2_TILENUM_K)+smo*(param.L2_TILENUM_K)+kmo)
				*(param.C_L2/ARRAY_C)*param.K_L2*param.R_L2*param.S_L2 + (co*param.K_L2*param.R_L2*param.S_L2+ro*param.S_L2*param.K_L2+so*param.K_L2+ko))*ARRAY_C + ci;
