...
void Conv_fifo(
		hls::stream<k2k_data> &bias_in, hls::stream<k2k_data> &weight_in,
		hls::stream<k2k_data> &data_in, hls::stream<k2k_data> &conv_out) {
#pragma HLS expression_balance
	...
	DPTYPE data_l1[512][ARRAY_C]; //double buffer
					...
					for (int ki = 0; ki < ARRAY_K; ki++) {
						...
						for (int hi = 0; hi < H_TILE; hi++) {
							for (int wi = 0; wi < W_TILE; wi++) {
								// TODO: WH TILE
								//output_l1[k * H_TILE * W_TILE + hi * W_TILE + wi] =
								//		bias_l2[k];
	...
	for (int k = 0; k < (K/ARRAY_K); k++) {
		for(int ki =0; ki<ARRAY_K; ki++) {
			for(int wh=0;wh<WH*WH;wh++) {
				output_tmp.data(31,0) = output_l1[k*WH*WH+wh][ki]; conv_out.write(output_tmp);
			}
		}
	}
